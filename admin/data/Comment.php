<?php
function fo($conn, $id, $user_id)
{
   $sql = "SELECT * FROM follow WHERE user_id=? AND following_by=?";
...
function likeCountByID($conn, $id){
...
function likeCount($conn, $id){
   $sql = "SELECT * FROM follow WHERE following_by=?";
   $stmt = $conn->prepare($sql);
   $stmt->execute([$id]);
...
function LikedByUserID($conn, $id, $user_id){
   $sql = "SELECT * FROM follow WHERE user_id=? AND following_by=?";
   $stmt = $conn->prepare($sql);
   $stmt->execute([$id, $user_id]);
...
function getCommentsByPostID1($conn, $id){
   $sql = "SELECT * FROM comment WHERE comment_id=?";
   $stmt = $conn->prepare($sql);
   $stmt->execute([$id]);

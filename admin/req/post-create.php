       ...
       if ($error === 0) {
           if ($image_size > 130000) {
              ...
              if (in_array($image_ex, $allowed_exs )) {
                  ...
                  $sql = "INSERT INTO post(post_title, post_text,category, cover_url, username) VALUES (?,?,?,?,?)";
                  $stmt = $conn->prepare($sql);
                  $res = $stmt->execute([$title, $text, $category, $new_image_name, $userid]);
              }else {
          ...
          $sql = "INSERT INTO post(post_title, post_text, category, username) VALUES (?,?,?,?)";
          $stmt = $conn->prepare($sql);
          $res = $stmt->execute([$title, $text, $category, $userid]);
      }

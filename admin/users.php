				...
				<table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 max-w-[1100px]">
					<thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
						<tr>
							...
							<th class="text-center px-6 py-3" scope="col">Full Name</th>
							<th class="text-center px-6 py-3" scope="col">username</th>
							<th class="text-center px-6 py-3" scope="col">Total Tweets</th>
					...
					<tbody>
						<?php foreach ($users as $user) { ?>
							<tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
								<th class="text-center px-6 py-4" scope="row"><?= $user['id'] ?></th>
								<td class="text-center px-6 py font-bold text-lg text-gray-900 whitespace-nowrap dark:text-white"><?= $user['fname'] ?></td>
								<td class="text-center px-6 py-4"><?= $user['username'] ?></td>
								<td class="text-center px-6 py font-bold text-lg text-gray-900 whitespace-nowrap dark:text-white"><?= totaltweet($conn , $user['username']) ?></td>
								<td class="text-center px-6 py-4">
									<a href="user-delete.php?user_id=<?= $user['id'] ?>" class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Delete</a>
								</td>
							</tr>
						<?php } ?>
